	...
	<div class="span12">
        ...
        foreach ($files as $file_array) {
            ...
            ?>
                ...
                <a class="folder-link" href="dialog.php?<?php echo $get_params.rawurlencode($src)."&".($callback?'callback='.$callback."&":'').uniqid() ?>">
                    <div class="img-precontainer">
                            ...
                            <img class="directory-img" data-src="img/<?php echo $config['icon_theme'];?>/folder<?php if($file==".."){ echo "_back"; }?>.png" src="img/<?php echo $config['icon_theme'];?>/folder<?php if($file==".."){ echo "_back"; }?>.png"/>
                            </div>
                    </div>
            ...
            foreach ($files as $nu=>$file_array) {
                ...
                if(!$ftp){
                    $file_path=$config['preview_path'].$rfm_subfolder.$subdir.$file;
                    //check if file have illegal caracter

                    if($file!=fix_filename($file,$config)){
                        $file1=fix_filename($file,$config);
                        $file_path1=($config['preview_path'].$rfm_subfolder.$subdir.$file1);
                        if(file_exists($file_path1)){
                            $i = 1;
                            $info=pathinfo($file1);
                            while(file_exists($config['preview_path'].$rfm_subfolder.$subdir.$info['filename'].".[".$i."].".$info['extension'])) {
                                $i++;
                            }
...
?>
            ...
            $file_prevent_delete = isset($filePermissions[$file]['prevent_delete']) && $filePermissions[$file]['prevent_delete'];
                ...
                /**
                 *  ******************** MODIFIÉ ********************
                 * We remove the public folder from the path and change it to absolute url.
                 * Ex:
                 *  <img src="../../../../public/uploads/IMAGEFILE.JPG">
                 *
                 *                        BECOME
                 * <img src="/uploads/IMAGEFILE.JPG">
                 *
                 */
                $src = strstr($src, '/uploads', false);
                if(strpos($src_thumb, 'img/') !== false){
//                It is an icon not a thumb ex text or .pdf
                } else{
                    if(strstr($src_thumb, '/thumbs', false)!= "")
                    {
                        $src_thumb = strstr($src_thumb, '/thumbs', false);
                    }else{
                        $src_thumb = $src;
                    }
                    $mini_src = $src;
            ...
            ?>
<!--                DEBUG          -->
<!--                --><?php //echo "CurDir: ".getcwd ();?>
<!--                --><?php //echo "src: ". $src ;?>
<!--                --><?php //echo "src_thumb: ". $src_thumb ;?>
<!--                --><?php //echo "mini_src: ". $mini_src ;?>
<!--                --><?php //echo "strstrTHUMBS: ". strstr($src_thumb, '/thumbs', false); ;?>
<!--                --><?php //echo "strstrSRC: ". strstr($src, '/uploads', false); ;?>
<!--               --><?php //******************** MODIFIÉ ********************?>
            ...
            <?php if($config['multiple_selection']){ ?><div class="selector">
                ...
                <div class="img-precontainer">
                    ...
                    <div class="img-container">
                        <img class="<?php echo $show_original ? "original" : "" ?><?php echo $is_icon_thumb ? " icon" : "" ?>" data-src="<?php echo $src_thumb;?>" src="<?php echo $src_thumb;?>" >
                    </div>
                ...
                <div class="img-precontainer-mini <?php if($is_img) echo 'original-thumb' ?>">
                    ...
                    <div class="img-container-mini">
                    <?php if($mini_src!=""){ ?>
                    <img class="<?php echo $show_original_mini ? "original" : "" ?><?php echo $is_icon_thumb_mini ? " icon" : "" ?>" data-src="<?php echo $mini_src;?>" src="<?php echo $mini_src;?>">
                    <?php } ?>
                    </div>
                </div>
